.class public Lorg/joda/time/format/o$g;
.super Ljava/lang/Object;
.source "SourceFile"

# interfaces
.implements Lorg/joda/time/format/q;
.implements Lorg/joda/time/format/p;


# annotations
.annotation system Ldalvik/annotation/EnclosingClass;
    value = Lorg/joda/time/format/o;
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x9
    name = "g"
.end annotation


# instance fields
.field public final a:Ljava/lang/String;

.field public final b:Ljava/lang/String;

.field public final c:[Ljava/lang/String;

.field public final d:Z

.field public final e:Z

.field public final f:Lorg/joda/time/format/q;

.field public volatile g:Lorg/joda/time/format/q;

.field public final h:Lorg/joda/time/format/p;

.field public volatile i:Lorg/joda/time/format/p;


# direct methods
.method public constructor <init>(Ljava/lang/String;Ljava/lang/String;[Ljava/lang/String;Lorg/joda/time/format/q;Lorg/joda/time/format/p;ZZ)V
    .locals 2

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    iput-object p1, p0, Lorg/joda/time/format/o$g;->a:Ljava/lang/String;

    iput-object p2, p0, Lorg/joda/time/format/o$g;->b:Ljava/lang/String;

    if-eqz p2, :cond_0

    invoke-virtual {p1, p2}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v0

    if-eqz v0, :cond_1

    :cond_0
    if-eqz p3, :cond_3

    array-length v0, p3

    if-nez v0, :cond_1

    goto :goto_1

    :cond_1
    new-instance v0, Ljava/util/TreeSet;

    sget-object v1, Ljava/lang/String;->CASE_INSENSITIVE_ORDER:Ljava/util/Comparator;

    invoke-direct {v0, v1}, Ljava/util/TreeSet;-><init>(Ljava/util/Comparator;)V

    invoke-virtual {v0, p1}, Ljava/util/TreeSet;->add(Ljava/lang/Object;)Z

    invoke-virtual {v0, p2}, Ljava/util/TreeSet;->add(Ljava/lang/Object;)Z

    if-eqz p3, :cond_2

    array-length p1, p3

    :goto_0
    add-int/lit8 p1, p1, -0x1

    if-ltz p1, :cond_2

    aget-object p2, p3, p1

    invoke-virtual {v0, p2}, Ljava/util/TreeSet;->add(Ljava/lang/Object;)Z

    goto :goto_0

    :cond_2
    new-instance p1, Ljava/util/ArrayList;

    invoke-direct {p1, v0}, Ljava/util/ArrayList;-><init>(Ljava/util/Collection;)V

    invoke-static {p1}, Ljava/util/Collections;->reverse(Ljava/util/List;)V

    invoke-virtual {p1}, Ljava/util/ArrayList;->size()I

    move-result p2

    new-array p2, p2, [Ljava/lang/String;

    invoke-virtual {p1, p2}, Ljava/util/ArrayList;->toArray([Ljava/lang/Object;)[Ljava/lang/Object;

    move-result-object p1

    check-cast p1, [Ljava/lang/String;

    iput-object p1, p0, Lorg/joda/time/format/o$g;->c:[Ljava/lang/String;

    goto :goto_2

    :cond_3
    :goto_1
    filled-new-array {p1}, [Ljava/lang/String;

    move-result-object p1

    iput-object p1, p0, Lorg/joda/time/format/o$g;->c:[Ljava/lang/String;

    :goto_2
    iput-object p4, p0, Lorg/joda/time/format/o$g;->f:Lorg/joda/time/format/q;

    iput-object p5, p0, Lorg/joda/time/format/o$g;->h:Lorg/joda/time/format/p;

    iput-boolean p6, p0, Lorg/joda/time/format/o$g;->d:Z

    iput-boolean p7, p0, Lorg/joda/time/format/o$g;->e:Z

    return-void
.end method

.method public static synthetic e(Lorg/joda/time/format/o$g;)Lorg/joda/time/format/p;
    .locals 0

    iget-object p0, p0, Lorg/joda/time/format/o$g;->i:Lorg/joda/time/format/p;

    return-object p0
.end method

.method public static synthetic f(Lorg/joda/time/format/o$g;)Lorg/joda/time/format/q;
    .locals 0

    iget-object p0, p0, Lorg/joda/time/format/o$g;->g:Lorg/joda/time/format/q;

    return-object p0
.end method


# virtual methods
.method public a(Lorg/joda/time/e;Ljava/lang/String;ILjava/util/Locale;)I
    .locals 17

    move-object/from16 v0, p0

    move-object/from16 v1, p1

    move-object/from16 v8, p2

    move/from16 v2, p3

    move-object/from16 v9, p4

    iget-object v3, v0, Lorg/joda/time/format/o$g;->h:Lorg/joda/time/format/p;

    invoke-interface {v3, v1, v8, v2, v9}, Lorg/joda/time/format/p;->a(Lorg/joda/time/e;Ljava/lang/String;ILjava/util/Locale;)I

    move-result v10

    if-gez v10, :cond_0

    return v10

    :cond_0
    const/4 v11, 0x0

    if-le v10, v2, :cond_4

    iget-object v12, v0, Lorg/joda/time/format/o$g;->c:[Ljava/lang/String;

    array-length v13, v12

    move v14, v11

    :goto_0
    if-ge v14, v13, :cond_4

    aget-object v15, v12, v14

    if-eqz v15, :cond_2

    invoke-virtual {v15}, Ljava/lang/String;->length()I

    move-result v2

    if-eqz v2, :cond_2

    const/4 v6, 0x0

    invoke-virtual {v15}, Ljava/lang/String;->length()I

    move-result v7

    const/4 v3, 0x1

    move-object/from16 v2, p2

    move v4, v10

    move-object v5, v15

    invoke-virtual/range {v2 .. v7}, Ljava/lang/String;->regionMatches(ZILjava/lang/String;II)Z

    move-result v2

    if-eqz v2, :cond_1

    goto :goto_1

    :cond_1
    add-int/lit8 v14, v14, 0x1

    goto :goto_0

    :cond_2
    :goto_1
    if-nez v15, :cond_3

    goto :goto_2

    :cond_3
    invoke-virtual {v15}, Ljava/lang/String;->length()I

    move-result v11

    :goto_2
    add-int/2addr v10, v11

    const/4 v2, 0x1

    move/from16 v16, v11

    move v11, v2

    move/from16 v2, v16

    goto :goto_3

    :cond_4
    const/4 v2, -0x1

    :goto_3
    iget-object v3, v0, Lorg/joda/time/format/o$g;->i:Lorg/joda/time/format/p;

    invoke-interface {v3, v1, v8, v10, v9}, Lorg/joda/time/format/p;->a(Lorg/joda/time/e;Ljava/lang/String;ILjava/util/Locale;)I

    move-result v1

    if-gez v1, :cond_5

    return v1

    :cond_5
    if-eqz v11, :cond_6

    if-ne v1, v10, :cond_6

    if-lez v2, :cond_6

    not-int v0, v10

    return v0

    :cond_6
    if-le v1, v10, :cond_7

    if-nez v11, :cond_7

    iget-boolean v0, v0, Lorg/joda/time/format/o$g;->d:Z

    if-nez v0, :cond_7

    not-int v0, v10

    return v0

    :cond_7
    return v1
.end method

.method public b(Lorg/joda/time/j;ILjava/util/Locale;)I
    .locals 1

    iget-object v0, p0, Lorg/joda/time/format/o$g;->f:Lorg/joda/time/format/q;

    invoke-interface {v0, p1, p2, p3}, Lorg/joda/time/format/q;->b(Lorg/joda/time/j;ILjava/util/Locale;)I

    move-result v0

    if-ge v0, p2, :cond_0

    iget-object p0, p0, Lorg/joda/time/format/o$g;->g:Lorg/joda/time/format/q;

    invoke-interface {p0, p1, p2, p3}, Lorg/joda/time/format/q;->b(Lorg/joda/time/j;ILjava/util/Locale;)I

    move-result p0

    add-int/2addr v0, p0

    :cond_0
    return v0
.end method

.method public c(Ljava/lang/StringBuffer;Lorg/joda/time/j;Ljava/util/Locale;)V
    .locals 4

    iget-object v0, p0, Lorg/joda/time/format/o$g;->f:Lorg/joda/time/format/q;

    iget-object v1, p0, Lorg/joda/time/format/o$g;->g:Lorg/joda/time/format/q;

    invoke-interface {v0, p1, p2, p3}, Lorg/joda/time/format/q;->c(Ljava/lang/StringBuffer;Lorg/joda/time/j;Ljava/util/Locale;)V

    iget-boolean v2, p0, Lorg/joda/time/format/o$g;->d:Z

    const/4 v3, 0x1

    if-eqz v2, :cond_2

    invoke-interface {v0, p2, v3, p3}, Lorg/joda/time/format/q;->b(Lorg/joda/time/j;ILjava/util/Locale;)I

    move-result v0

    if-lez v0, :cond_3

    iget-boolean v0, p0, Lorg/joda/time/format/o$g;->e:Z

    if-eqz v0, :cond_1

    const/4 v0, 0x2

    invoke-interface {v1, p2, v0, p3}, Lorg/joda/time/format/q;->b(Lorg/joda/time/j;ILjava/util/Locale;)I

    move-result v0

    if-lez v0, :cond_3

    if-le v0, v3, :cond_0

    iget-object p0, p0, Lorg/joda/time/format/o$g;->a:Ljava/lang/String;

    goto :goto_0

    :cond_0
    iget-object p0, p0, Lorg/joda/time/format/o$g;->b:Ljava/lang/String;

    :goto_0
    invoke-virtual {p1, p0}, Ljava/lang/StringBuffer;->append(Ljava/lang/String;)Ljava/lang/StringBuffer;

    goto :goto_1

    :cond_1
    iget-object p0, p0, Lorg/joda/time/format/o$g;->a:Ljava/lang/String;

    invoke-virtual {p1, p0}, Ljava/lang/StringBuffer;->append(Ljava/lang/String;)Ljava/lang/StringBuffer;

    goto :goto_1

    :cond_2
    iget-boolean v0, p0, Lorg/joda/time/format/o$g;->e:Z

    if-eqz v0, :cond_3

    invoke-interface {v1, p2, v3, p3}, Lorg/joda/time/format/q;->b(Lorg/joda/time/j;ILjava/util/Locale;)I

    move-result v0

    if-lez v0, :cond_3

    iget-object p0, p0, Lorg/joda/time/format/o$g;->a:Ljava/lang/String;

    invoke-virtual {p1, p0}, Ljava/lang/StringBuffer;->append(Ljava/lang/String;)Ljava/lang/StringBuffer;

    :cond_3
    :goto_1
    invoke-interface {v1, p1, p2, p3}, Lorg/joda/time/format/q;->c(Ljava/lang/StringBuffer;Lorg/joda/time/j;Ljava/util/Locale;)V

    return-void
.end method

.method public d(Lorg/joda/time/j;Ljava/util/Locale;)I
    .locals 5

    iget-object v0, p0, Lorg/joda/time/format/o$g;->f:Lorg/joda/time/format/q;

    iget-object v1, p0, Lorg/joda/time/format/o$g;->g:Lorg/joda/time/format/q;

    invoke-interface {v0, p1, p2}, Lorg/joda/time/format/q;->d(Lorg/joda/time/j;Ljava/util/Locale;)I

    move-result v2

    invoke-interface {v1, p1, p2}, Lorg/joda/time/format/q;->d(Lorg/joda/time/j;Ljava/util/Locale;)I

    move-result v3

    add-int/2addr v2, v3

    iget-boolean v3, p0, Lorg/joda/time/format/o$g;->d:Z

    const/4 v4, 0x1

    if-eqz v3, :cond_2

    invoke-interface {v0, p1, v4, p2}, Lorg/joda/time/format/q;->b(Lorg/joda/time/j;ILjava/util/Locale;)I

    move-result v0

    if-lez v0, :cond_3

    iget-boolean v0, p0, Lorg/joda/time/format/o$g;->e:Z

    if-eqz v0, :cond_1

    const/4 v0, 0x2

    invoke-interface {v1, p1, v0, p2}, Lorg/joda/time/format/q;->b(Lorg/joda/time/j;ILjava/util/Locale;)I

    move-result p1

    if-lez p1, :cond_3

    if-le p1, v4, :cond_0

    iget-object p0, p0, Lorg/joda/time/format/o$g;->a:Ljava/lang/String;

    goto :goto_0

    :cond_0
    iget-object p0, p0, Lorg/joda/time/format/o$g;->b:Ljava/lang/String;

    :goto_0
    invoke-virtual {p0}, Ljava/lang/String;->length()I

    move-result p0

    goto :goto_1

    :cond_1
    iget-object p0, p0, Lorg/joda/time/format/o$g;->a:Ljava/lang/String;

    invoke-virtual {p0}, Ljava/lang/String;->length()I

    move-result p0

    :goto_1
    add-int/2addr v2, p0

    goto :goto_2

    :cond_2
    iget-boolean v0, p0, Lorg/joda/time/format/o$g;->e:Z

    if-eqz v0, :cond_3

    invoke-interface {v1, p1, v4, p2}, Lorg/joda/time/format/q;->b(Lorg/joda/time/j;ILjava/util/Locale;)I

    move-result p1

    if-lez p1, :cond_3

    iget-object p0, p0, Lorg/joda/time/format/o$g;->a:Ljava/lang/String;

    invoke-virtual {p0}, Ljava/lang/String;->length()I

    move-result p0

    goto :goto_1

    :cond_3
    :goto_2
    return v2
.end method

.method public g(Lorg/joda/time/format/q;Lorg/joda/time/format/p;)Lorg/joda/time/format/o$g;
    .locals 0

    iput-object p1, p0, Lorg/joda/time/format/o$g;->g:Lorg/joda/time/format/q;

    iput-object p2, p0, Lorg/joda/time/format/o$g;->i:Lorg/joda/time/format/p;

    return-object p0
.end method
